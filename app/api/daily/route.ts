...
// Public route — players use this to get today's challenge and submit answers
...
export async function GET() {
  try {
    ...
    // Get today's challenge — strip correct_answer for security
    ...
    // Check if logged-in user has already completed today
    let completed = false;
    let isCorrect = null;
    let dailyStreak = 0;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get correct answer server-side
    ...
    const { data: profile } = await supabase
      .from("users")
      .select("daily_streak, last_daily_date, xp")
      .eq("id", user.id)
      .single();
